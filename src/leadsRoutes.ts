...
router.post('/', (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const lead = db.prepare('SELECT * FROM leads WHERE id = ?').get(result.lastInsertRowid) as any;
...
// Update lead stage
router.post('/:id/stage', (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
    ...
    const lead = db.prepare('SELECT * FROM leads WHERE id = ? AND userId = ?').get(id, req.user.id);
    if (!lead) {
...
router.post('/:id/activity', (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
...
// Get lead history
router.get('/:id/history', (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
...
router.delete('/:id', (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    db.prepare('DELETE FROM leads WHERE id = ? AND userId = ?').run(id, req.user.id);
