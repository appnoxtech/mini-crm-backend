...
export class ReminderService {
    ...
    async addReminder(eventId: number, userId: number, minutesBefore: number): Promise<EventReminder | null> {
        ...
        const event = await this.eventModel.findById(eventId, userId);
        if (!event) return null;

        // Remove default reminder when adding custom
        await this.reminderModel.deleteDefaultByEventId(eventId);
    ...
    async updateReminder(reminderId: number, userId: number, minutesBefore: number): Promise<EventReminder | null> {
        ...
        // Verify user owns the event
        const event = await this.eventModel.findById(reminder.eventId, userId);
        if (!event) return null;

        const updatedReminder = await this.reminderModel.update(reminderId, { reminderMinutesBefore: minutesBefore });
        if (!updatedReminder) return null;

        // Reschedule notifications
        const reminders = await this.reminderModel.findByEventId(reminder.eventId);
        await this.notificationScheduler.rescheduleForEvent(event, reminders);
    ...
    async deleteReminder(reminderId: number, userId: number): Promise<boolean> {
        ...
        await this.notificationScheduler.removeForReminder(reminderId);
        const deleted = await this.reminderModel.delete(reminderId);

        if (deleted) {
            // If no custom reminders left, add default back
            await this.reminderModel.ensureDefaultReminder(reminder.eventId, 30);
    ...
    async getReminders(eventId: number): Promise<EventReminder[]> {
        return await this.reminderModel.findByEventId(eventId);
    }
}
