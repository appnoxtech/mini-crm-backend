...
export class EmailService {
  ...
  constructor(emailModel: EmailModel, connectorService: EmailConnectorService, notificationService?: RealTimeNotificationService) {
  ...
  async processIncomingEmails(account: EmailAccount): Promise<{ processed: number; errors: number }> {
    ...
    try {
      ...
      if (provider === 'gmail') {
        rawEmails = await this.connectorService.fetchGmailEmails(account, account.lastSyncAt);
      ...
      await this.emailModel.updateEmailAccount(account.id, { lastSyncAt: new Date() });
  ...
  private async processSingleEmail(account: EmailAccount, rawEmail: any): Promise<void> {
  ...
  private determineEmailDirection(parsed: Partial<Email>, account: EmailAccount, rawEmail: any): boolean {
  ...
  private determineGmailEmailDirection(parsed: Partial<Email>, account: EmailAccount, rawEmail: any): boolean {
  ...
  private determineOutlookEmailDirection(parsed: Partial<Email>, account: EmailAccount, rawEmail: any): boolean {
  ...
  private determineIMAPEmailDirection(parsed: Partial<Email>, account: EmailAccount, rawEmail: any): boolean {
  ...
  private parseGmailMessage(message: any): Partial<Email> {
    ...
    return {
      ...
      from: headers['from'],
      to: (headers['to'] || '').split(',').filter(Boolean).map((s: string) => s.trim()),
      cc: headers['cc'] ? headers['cc'].split(',').map((s: string) => s.trim()) : undefined,
      bcc: headers['bcc'] ? headers['bcc'].split(',').map((s: string) => s.trim()) : undefined,
  ...
  private parseIMAPMessage(message: any): Partial<Email> {
    return {
      ...
      isRead: !(message.flags && message.flags.has && message.flags.has('\\Seen') === false)
    } as any;
  }
  ...
  async getEmailsForUser(userId: string, options: {
    limit?: number;
    offset?: number;
    folder?: string;
    search?: string;
