...
export class EmailModel {
  ...
  async getEmailAccounts(userId: string): Promise<EmailAccount[]> {
    const stmt = this.db.prepare('SELECT * FROM email_accounts WHERE userId = ? ORDER BY createdAt DESC');
  ...
  async getEmailsForUser(userId: string, options: {
    ...
    const { limit = 50, offset = 0, folder = 'inbox', search, unreadOnly = false } = options;
