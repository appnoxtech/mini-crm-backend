...
class ImportController {
    ...
    uploadFile = async (req, res) => {
        try {
            ...
            const preview = await this.importService.uploadFile(userId, {
    ...
    startImport = async (req, res) => {
        try {
            ...
            const result = await this.importService.startImport(userId, importId, { duplicateHandling: duplicateHandling });
            res.json({ success: true, data: result });
        }
        catch (error) {
     ...
     */
    getFieldDefinitions = async (req, res) => {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({ success: false, error: 'Unauthorized' });
