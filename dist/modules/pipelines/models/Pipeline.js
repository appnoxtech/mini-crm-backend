...
class PipelineModel {
    ...
    create(data) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        const result = stmt.run(data.name, data.description || null, data.userId, data.isDefault ? 1 : 0, data.isActive ? 1 : 0, data.dealRotting ? 1 : 0, data.rottenDays, now, now);
    ...
    findByUserId(userId, includeInactive = false) {
    ...
    delete(id, userId) {
        ...
        const dealCount = this.db.prepare('SELECT COUNT(*) as count FROM deals WHERE pipelineId = ?').get(id);
    ...
    getStats(pipelineId) {
      ...
      WHERE pipelineId = ?
    `);
        const result = stmt.get(pipelineId);
        return {
...
exports.PipelineModel = PipelineModel;
