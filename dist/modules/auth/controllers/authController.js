...
class AuthController {
    ...
    async register(req, res) {
        try {
            ...
            if (existingUser) {
                return responses_1.ResponseHandler.conflict(res, 'User already exists', { email });
            }
            // Create user using auth service
            const user = await this.authService.createUser(email, name, password);
            //const token = this.authService.generateToken(user);
            if (!user) {
                return responses_1.ResponseHandler.error(res, "Failed to create user", 500);
            }
            return responses_1.ResponseHandler.created(res, user, 'User Registered Successfully');
        }
        catch (error) {
            return responses_1.ResponseHandler.internalError(res, 'Registration failed');
        }
    }
    async login(req, res) {
        try {
            ...
            if (!user) {
                return responses_1.ResponseHandler.error(res, 'Invalid email or password');
            }
            const token = this.authService.generateToken(user);
            ...
            return responses_1.ResponseHandler.success(res, { user, token, emailAccount, emailSetupStatus }, "Login Successfully");
        }
        catch (error) {
            console.error('Login error:', error);
            return responses_1.ResponseHandler.internalError(res, 'Login Failed');
        }
    }
    async getProfile(req, res) {
        try {
            ...
            if (!tokenUser) {
                return responses_1.ResponseHandler.forbidden(res, 'User not authenticated');
            }
            const user = await this.authService.getProfile(tokenUser.id);
            if (!user) {
                return responses_1.ResponseHandler.notFound(res, 'User not found');
            }
            return responses_1.ResponseHandler.success(res, user);
        }
        catch (error) {
            console.error('Profile error:', error);
            return responses_1.ResponseHandler.internalError(res, 'Failed to get profile');
        }
    }
    async updateProfile(req, res) {
        try {
            ...
            if (name !== undefined) {
                if (!name.trim()) {
                    return responses_1.ResponseHandler.validationError(res, 'Name cannot be empty');
                }
                updates.name = name.trim();
            }
            if (email !== undefined) {
                if (!email.includes('@')) {
                    return responses_1.ResponseHandler.validationError(res, 'Please enter a valid email address');
                }
            ...
            if (!updatedUser) {
                return responses_1.ResponseHandler.validationError(res, 'User not found');
            }
            return responses_1.ResponseHandler.success(res, updatedUser, "Profile Update Successfully", 200);
        }
        catch (error) {
    ...
    async changePassword(req, res) {
        try {
            ...
            if (!success) {
                return responses_1.ResponseHandler.error(res, 'Current password is incorrect', 401);
            }
            return responses_1.ResponseHandler.success(res, 'Password updated successfully');
        }
        catch (error) {
