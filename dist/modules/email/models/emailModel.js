...
class EmailModel {
    ...
    `);
      ...
      `);
        ...
        this.db.exec("CREATE INDEX IF NOT EXISTS idx_email_accounts_userId ON email_accounts(userId)");
    ...
    `);
        stmt.run(email.id, email.messageId, email.threadId || null, email.accountId, email.from, JSON.stringify(email.to), email.cc ? JSON.stringify(email.cc) : null, email.bcc ? JSON.stringify(email.bcc) : null, email.subject, email.body, email.htmlBody || null, email.isRead ? 1 : 0, email.isIncoming ? 1 : 0, email.sentAt.toISOString(), email.receivedAt?.toISOString() || null, JSON.stringify(email.contactIds), JSON.stringify(email.dealIds), JSON.stringify(email.accountEntityIds), email.trackingPixelId || null, email.opens, email.clicks, email.createdAt.toISOString(), email.updatedAt.toISOString(), email.labelIds ? JSON.stringify(email.labelIds) : null);
        return email;
    }
    ...
    async getEmailAccountById(accountId) {
        const stmt = this.db.prepare("SELECT * FROM email_accounts WHERE id = ? AND isActive = 1");
        ...
        if (row.accessToken)
            account.accessToken = row.accessToken;
        if (row.refreshToken)
            account.refreshToken = row.refreshToken;
        if (row.imapConfig)
            account.imapConfig = JSON.parse(row.imapConfig);
        if (row.smtpConfig)
    ...
    async getEmailAccountByUserId(userId) {
        const stmt = this.db.prepare("SELECT * FROM email_accounts WHERE userId = ? AND isActive = 1 LIMIT 1");
    ...
    async getEmailAccountByEmail(email) {
        const stmt = this.db.prepare("SELECT * FROM email_accounts WHERE email = ? AND isActive = 1 LIMIT 1");
    ...
    async updateEmailAccount(accountId, updates) {
        ...
        const stmt = this.db.prepare(`UPDATE email_accounts SET ${fields.join(", ")} WHERE id = ?`);
        stmt.run(...values);
    }
    async getAllActiveAccounts() {
        const stmt = this.db.prepare("SELECT * FROM email_accounts WHERE isActive = 1");
        const rows = stmt.all();
        return rows.map((row) => {
            const account = {
                id: row.id,
    ...
    async getEmailAccounts(userId) {
        ...
        const stmt = this.db.prepare("SELECT * FROM email_accounts WHERE userId = ? AND isActive = 1 ORDER BY createdAt DESC");
    ...
    async saveThreadSummary(threadId, summary) {
        const stmt = this.db.prepare(`
  ...
  `);
    ...
    async getThreadsNeedingSummary() {
        const stmt = this.db.prepare(`
  ...
  `);
    ...
    async findDealsByContactIds(contactIds) {
        // Mock implementation
        return [];
    }
    ...
    async getEmailsForUser(userId, options = {}) {
        const { limit = 50, offset = 0, folder = "inbox", search, unreadOnly = false, accountId, } = options;
        // Build the WHERE clause
        let whereClause = "WHERE e.accountId IN (SELECT id FROM email_accounts WHERE userId = ?)";
        const params = [userId];
        if (accountId) {
        ...
        // Get total count
        const countStmt = this.db.prepare(`SELECT COUNT(*) as total FROM emails e ${whereClause}`);
    ...
    `;
        ...
        const emailRows = emailsStmt.all(...params, limit, offset);
    ...
    `);
        const row = stmt.get(emailId, userId);
        if (!row)
            return null;
        return {
            id: row.id,
    ...
    `);
        const result = stmt.run(isRead ? 1 : 0, new Date().toISOString(), emailId, userId);
