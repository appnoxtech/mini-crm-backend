...
class DraftModel {
    ...
    `);
        stmt.run(id, input.accountId, userId, JSON.stringify(input.to), input.cc ? JSON.stringify(input.cc) : null, input.bcc ? JSON.stringify(input.bcc) : null, input.subject, input.body, input.htmlBody || null, input.attachments ? JSON.stringify(input.attachments) : null, input.replyToMessageId || null, input.forwardFromMessageId || null, input.threadId || null, input.contactIds ? JSON.stringify(input.contactIds) : null, input.dealIds ? JSON.stringify(input.dealIds) : null, input.accountEntityIds ? JSON.stringify(input.accountEntityIds) : null, input.enableTracking ? 1 : 0, input.isScheduled ? 1 : 0, input.scheduledFor ? input.scheduledFor.toISOString() : null, now, now);
    ...
    `);
        const row = stmt.get(draftId, userId);
        if (!row)
            return null;
    ...
    async listDrafts(userId, options = {}) {
        const { limit = 50, offset = 0, search, accountId, scheduledOnly = false, } = options;
     ...
     */
    async getScheduledDraftsReadyToSend() {
        const now = new Date().toISOString();
        const stmt = this.db.prepare(`
      SELECT * FROM email_drafts
