...
class EmailService {
    ...
    async processIncomingEmails(account) {
        ...
        try {
            ...
            console.log(`Email processing completed. Processed: ${processed}, New: ${newEmails}, Errors: ${errors}`);
    ...
    async processSingleEmail(account, rawEmail) {
        ...
        if (this.notificationService && isIncoming) {
            console.log(`🔔 Triggering WebSocket notification for: ${email.subject} (User: ${account.userId})`);
     ...
     */
    async syncArchivedEmails(userId) {
    ...
    async syncGmailHistory(account) {
    ...
    async ensureArchiveStatus(emailId, userId) {
