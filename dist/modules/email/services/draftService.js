...
class DraftService {
    draftModel;
    emailService;
    constructor(draftModel, emailService) {
    ...
    async createDraft(userId, input) {
        ...
        if (input.cc)
            this.validateEmailAddresses(input.cc, 'cc');
        if (input.bcc)
            this.validateEmailAddresses(input.bcc, 'bcc');
    ...
    async updateDraft(draftId, userId, updates) {
        ...
        if (updates.cc)
            this.validateEmailAddresses(updates.cc, 'cc');
        if (updates.bcc)
            this.validateEmailAddresses(updates.bcc, 'bcc');
    ...
    async sendDraft(draftId, userId) {
        ...
        try {
            ...
            const messageId = await this.emailService.sendEmail(draft.accountId, {
                to: draft.to,
                cc: draft.cc,
                bcc: draft.bcc,
     ...
     */
    validateEmailAddresses(emails, field) {
     ...
     */
    async duplicateDraft(draftId, userId) {
