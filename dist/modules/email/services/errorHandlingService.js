...
class ErrorHandlingService {
    ...
    classifyGmailApiError(gmailError) {
        ...
        switch (code) {
            ...
            case 403:
                if (details?.some((d) => d.reason === 'quotaExceeded')) {
                    return {
                        is_retryable: true,
                        is_permanent: false,
                        is_temporary: true,
                ...
                else if (details?.some((d) => d.reason === 'insufficientPermissions')) {
                    return {
                        is_retryable: true,
                        is_permanent: false,
                        is_temporary: false,
    ...
    makeRetryDecision(errorScenario) {
    ...
    calculateExponentialBackoff(attempt, multiplier) {
    ...
    addToDeadLetterQueue(messageId, errorScenario, originalRequest) {
    ...
    isCircuitBreakerOpen(serviceKey) {
    ...
    recordCircuitBreakerFailure(serviceKey) {
    ...
    getCircuitBreakerStatus(serviceKey) {
        const circuit = this.circuitBreakers.get(serviceKey);
        if (!circuit) {
    ...
    getErrorStatistics(timeframe = 'day') {
        const now = Date.now();
    ...
    extractErrorCategory(failureReason) {
        if (failureReason.includes('authentication'))
            return 'authentication';
        if (failureReason.includes('quota'))
            return 'quota';
        if (failureReason.includes('rate_limit'))
            return 'rate_limit';
        if (failureReason.includes('network'))
            return 'network';
        if (failureReason.includes('server'))
            return 'server';
        return 'unknown';
    }
}
