...
exports.EmailConnectorService = void 0;
...
class EmailConnectorService {
    ...
    async connectGmail(account) {
        ...
        this.gmailClient = googleapis_1.google.gmail({ version: 'v1', auth });
    }
    async connectOutlook(account) {
        ...
        this.outlookClient = Client.init({
            authProvider: ({ scopes }) => Promise.resolve(account.accessToken)
        });
    }
    ...
    async fetchGmailEmails(account, lastSyncTime, maxResults = 50) {
        try {
    ...
    async fetchIMAPEmails(account, lastSyncTime) {
        const client = await this.connectIMAP(account);
        const messages = [];
        const searchCriteria = lastSyncTime ? { since: lastSyncTime } : { all: true };
        try {
            // 1. Fetch from INBOX
            ...
            const sentBox = mailboxes.find((box) => (box.specialUse === '\\Sent' || box.name === 'Sent' || box.name === 'Sent Items' || box.name === 'Sent Mail'));
            if (sentBox) {
            ...
            const draftBox = mailboxes.find((box) => (box.specialUse === '\\Drafts' || box.name.toLowerCase().includes('draft')));
            if (draftBox) {
                try {
            ...
            const spamBox = mailboxes.find((box) => (box.specialUse === '\\Junk' || box.name.toLowerCase().includes('spam') || box.name.toLowerCase().includes('junk')));
            if (spamBox) {
                try {
            ...
            const trashBox = mailboxes.find((box) => (box.specialUse === '\\Trash' || box.name.toLowerCase().includes('trash') || box.name.toLowerCase().includes('delete')));
            if (trashBox) {
                try {
    ...
    async validateOAuthTokens(account) {
    ...
    async verifyOAuthTokens(account) {
        if (account.provider === 'gmail') {
            try {
    ...
    async sendEmail(account, emailData) {
    ...
    async sendGmailEmailViaSMTP(account, emailData) {
            ...
            if (error.message.includes('invalid_request') ||
                ...
                try {
                    ...
                    if (refreshResult) {
                        ...
                        const refreshedTransporter = nodemailer_1.default.createTransport({
                            service: 'gmail',
                            auth: {
                                type: 'OAuth2',
    ...
    async sendOutlookEmailViaSMTP(account, emailData) {
    ...
    async sendGmailEmailViaAPI(account, emailData) {
        await this.connectGmail(account);
    ...
    async sendOutlookEmailViaAPI(account, emailData) {
        await this.connectOutlook(account);
        const message = {
            subject: emailData.subject,
            body: {
...
exports.EmailConnectorService = EmailConnectorService;
//# sourceMappingURL=emailConnectorService.js.map