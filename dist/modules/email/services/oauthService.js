...
class OAuthService {
    ...
    async handleGoogleCallback(code, state) {
        ...
        try {
            ...
            const gmail = googleapis_1.google.gmail({ version: 'v1', auth: this.googleOAuth2Client });
    ...
    async generateMicrosoftAuthUrl(userId) {
        if (!this.msalClient) {
            const missingVars = [];
            if (!process.env.MICROSOFT_CLIENT_ID)
                missingVars.push('MICROSOFT_CLIENT_ID');
            if (!process.env.MICROSOFT_CLIENT_SECRET)
                missingVars.push('MICROSOFT_CLIENT_SECRET');
    ...
    createEmailAccountFromOAuth(userId, email, provider, accessToken, refreshToken) {
        return {
    ...
    logOAuthActivity(userId, action, provider, success, details) {
        const logEntry = {
