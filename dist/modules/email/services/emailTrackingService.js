...
class EmailTrackingService {
    ...
    trackingBaseUrl;
    notificationService;
    constructor(trackingBaseUrl = process.env.TRACKING_BASE_URL || 'http://localhost:3001', notificationService) {
        this.trackingBaseUrl = trackingBaseUrl;
        this.notificationService = notificationService;
    }
    async initializeTracking(request, recipients) {
    ...
    async trackEmailOpen(messageId, recipientEmail, metadata) {
    ...
    async trackLinkClick(messageId, originalUrl, recipientEmail, metadata) {
    ...
    async updateDeliveryStatus(messageId, status, recipientEmail, reason) {
    ...
    updateCampaignStats(campaignId, action, count) {
    ...
    async getEngagementReport(timeframe = 'week') {
        const now = new Date();
